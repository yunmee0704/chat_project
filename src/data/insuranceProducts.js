 ...
 * 사용자 질문/챗봇 답변에 들어 있는 단어를 보고
 * 아래 목록 순서대로 첫 상품을 고릅니다.
 */
var SAMSUNG_LIFE = '삼성생명보험'
...
function copyCard(card) {
  return {
    bannerKicker: card.bannerKicker,
    bannerTitle: card.bannerTitle,
    productName: card.productName,
    description: card.description,
    premiumLabel: card.premiumLabel,
...
export function pickInsuranceProduct(userText, assistantText) {
  ...
  for (i = 0; i < INSURANCE_PRODUCTS.length; i += 1) {
    for (j = 0; j < INSURANCE_PRODUCTS[i].keywords.length; j += 1) {
      if (combined.includes(INSURANCE_PRODUCTS[i].keywords[j].toLowerCase())) {
        return copyCard(INSURANCE_PRODUCTS[i].card)
      }
    }
  }

  return copyCard(DEFAULT_INSURANCE_PRODUCT)
}
